...
% \sam{I'll skip the introduction for now} 
\sam{Quantum Machine Learning (QML) is an emerging area of quantum applications that aims to explore the impacts of quantum computing for learning tasks ~\cite{Huang2022Quantumadvantagein, Liu2021Arigorousandrobust}.}\sam{bring some references}.
%As Quantum Machine Learning (QML) matures, a growing body of work asks whether combining parameterised quantum circuits (PQCs) with established deep-learning components can yield behaviour that the classical components do not exhibit on their own. Hybrid quantum neural networks (HQNNs), in which the bulk of the model remains classical and only a small block is delegated to a quantum device, are widely regarded as the pragmatic formulation for noisy intermediate-scale quantum (NISQ) hardware~\cite{preskill2018}: they keep circuits shallow and narrow, and they leave the burden of representing high-dimensional raw features to classical layers that are known to do it well~\cite{benedetti2019}.

Reinforcement learning (RL) is an appealing target for this kind of
hybridisation, and a demanding one. It is appealing because an RL agent learns a policy from interaction rather than from labelled examples, which suits financial settings where the data-generating process drifts and where the
quantity of interest is a sequence of decisions rather than a single
prediction. It is demanding because RL training is notoriously
high-variance: the same architecture, trained twice under different random
initialisations, can produce policies whose realised returns differ by a wide
margin. This second property is the methodological crux of the present work.
In a domain where single runs are not informative, any claim about the effect
of a quantum layer that rests on a single quantum trajectory compared against a
single classical trajectory is not identifiable --- the difference could as
easily be an artefact of initialisation.

%
This research focus on three principal commitments, each of which shapes the
structure of the paper. First, the quantum and classical models must differ in
exactly one block, with identical interfaces, so that the comparison is
attributable to the parameterisation of that block. Second, every configuration
must be characterised by a \emph{distribution} over independent training runs
rather than by a point estimate. Third, the quantum block must be evaluated in
a regime in which its contribution can be separated from estimation noise; for
that reason the experiments reported here are conducted with exact
(statevector) expectation values, which correspond to the noiseless limit of
the circuit and isolate the architectural effect. Finite-shot estimation and
execution on hardware introduce a second, independent source of stochasticity
and are outside the scope of this study (\ref{sec:limitations}).

The narrative of the paper follows the order in which the experimental
decisions were actually taken:

\begin{enumerate}
  \item We adopt an existing RL framework for portfolio
        optimisation~\cite{RLportfolio} and survey the policy architectures it
        provides, selecting EI3~\cite{costa2023} --- a convolutional descendant
        of the EIIE formulation of Jiang et al.~\cite{jiang2017} --- as the
        baseline. The selection is supported by an experiment, not by
        assumption: EI3 is characterised across three training budgets to
        establish both its attainable level and its dispersion before any
        modification is introduced (\ref{subsec:res-baseline}).
  \item EI3 in its original form has no latent representation of a width we can
        control, and therefore no natural site for a quantum block. We derive a
        scaffold --- a latent projection with bounded angle encoding and an MLP decision head --- that creates such a site, and we justify the removal of the gated residual branch that an early hybrid design contained, on the grounds that a bypass path around the block destroys the identifiability of the ablation (\ref{subsec:design-constraints}).
  \item We then ask whether the scaffold alone accounts for any change, by
        evaluating it with the latent block removed entirely (the No-Layer
        variant), run by run (\ref{subsec:res-scaffold}).
  \item From the scaffold we derive two models that differ only in the
        parameterisation of the latent block: a classical dense layer
        (QEI3-eq) and a variational circuit (QEI3). We sweep the width of the
        classical block (\ref{subsec:res-classical-width}) and, under exact
        simulation, the width of the quantum register
        (\ref{subsec:res-qubit-width}), and finally compare the two at matched
        width (\ref{subsec:res-headtohead}).
\end{enumerate}

The contributions are: (i) a hybridisation-ready reformulation of EI3 together
with the design rationale that makes a latent-block ablation identifiable;
(ii) a repeated-run experimental protocol under which every configuration is
reported as a distribution rather than a point; (iii) an empirical
characterisation, in the noiseless limit, of what the variational block changes
--- principally the dispersion of the learned policy across initialisations;
and (iv) evidence that this effect is insensitive to register width over
\num{2}--\num{8} qubits, together with a theoretical reading of why that is the
expected behaviour for the encoding and readout adopted.
