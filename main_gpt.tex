...
width; and (iv) how the quantum configuration behaves when register width and
measurement sampling are varied. The experiments are intended as controlled
...
The exact statevector regime is used first to evaluate the implemented quantum
block without finite-shot sampling noise. Finite-shot behavior is considered
separately in the experiments reported later.

% GPT-REVISION: Removed "upper bound of what the block can contribute". Exact
% expectation values remove sampling noise, but they do not provide a general
% upper bound on learning performance.
\paragraph{Exact expectation-value regime.}
For a fixed circuit and observable, finite-shot estimates converge to the exact
expectation value as the number of measurements increases. The statevector
experiments therefore provide a deterministic expectation-value reference for
the implemented circuit, without shot noise. They do not model device noise,
finite sampling cost, queueing, compilation overhead, or other hardware effects.

\paragraph{Separating training variability from shot noise.}
...
spread confounds the architectural effect with the estimator variance. The exact regime removes this additional sampling contribution, which makes the
observed run-to-run dispersion easier to interpret. Finite-shot experiments are
then used to examine how measurement sampling changes that behavior.

% GPT-REVISION: Removed the statements that exact simulation is the "only" regime
% in which the ablation is identifiable and that finite shots make the claim
% "unfalsifiable"; both were stronger than the experimental design establishes.

\paragraph{Reproducibility of the simulation path.}
For fixed parameters, the statevector expectation calculation is deterministic.
The training pipeline additionally depends on the pseudorandom sources described
above.

\textcolor{red}{REVIEW: verify the claim of bit-for-bit reproducibility across
software versions, hardware backends and numerical libraries. Unless this was
tested explicitly, state only that the seeded software configuration is intended
to make repeated batches reproducible. Also verify the exact Qiskit version and
whether the sampling primitive used for the finite-shot experiments exposes a
seed in that version.}
