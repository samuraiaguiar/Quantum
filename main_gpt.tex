...
Because the variability of RL training across initialisations is of the same order as the effects under study, no configuration is characterized by a single
...
published for the reference model~\cite{RLportfolio}, and is used to characterise
...
implementation~\cite{RLportfolio} and shown in \ref{fig:arch-ei3}.
...
comparison. The sequence makes the changes between configurations explicit; because
not all campaigns are seed-paired, it should not be read as a causal decomposition
of performance.
...
as the strongest EI3 configuration measured in this experiment; this does not
establish that it is the globally optimal EI3 training configuration.

Second, the run-to-run dispersion is substantial in the evaluated EI3 configurations: standard deviations of \num{31608}, \num{28132} and \num{55409}, the last
corresponding to a coefficient of variation of roughly \num{0.25}. The variation across the 20 runs motivates reporting repeated-run distributions
rather than relying on a single trained agent.
...
The scaffold has a mean final value of \num{280563} monetary units, above the
highest mean observed for the three EI3 configurations reported above. This
comparison is descriptive rather than causal, and the scaffold also exhibits
substantial run-to-run dispersion: the standard deviation across runs is \num{82410}, a coefficient of
...
policies of a common shape or that initialization primarily rescales a common
policy, this requires a quantitative policy/trajectory-similarity analysis.
Without such an analysis, retain only the descriptive statement about the
observed dispersion.}
...
between-configuration inferential test for this comparison, so no statistical
difference is claimed.

The comparison therefore provides a classical reference for the quantum
configuration without establishing whether the classical latent block itself
causes an improvement.
\textcolor{red}{REVIEW: compute and report an appropriate between-configuration
test and effect-size estimate for QEI3-eq NoCL versus QEI3-eq if an inferential
claim about the classical latent block is intended.}
...
(Kruskal--Wallis $H(2) = 2.13$, $p = 0.345$, $\varepsilon^2 = 0.002$). The reported effect-size estimate is small. However, \ref{eq:seed-rule} ties
each width to a distinct pseudorandom stream, so register width and initialization
stream are confounded. The experiment therefore does not establish that width is
irrelevant; it shows only that no between-width difference is detected for these
three tested campaigns.
...
for the scaffold and \num{0.22} for its classical instantiation. These values are lower than those observed for the scaffold and the tested
classical latent block under the reported protocol. The present experiment does
not identify the mechanism responsible for this difference.

The two-qubit circuit uses fewer circuit parameters and entangling gates than
the wider circuits (four parameters and one CNOT, compared with sixteen
parameters and twenty-eight CNOTs at eight qubits). Since the final-value
distributions are not distinguished in this sweep, the results do not provide
evidence that the wider registers improve the reported portfolio metric.
...
Across the configurations listed in the table, the observed means and
dispersions follow an ordered pattern. Because the campaigns are not fully
seed-paired and the architectures differ in parameterization, this ordering is
reported descriptively and is not interpreted as a causal progression.
\begin{table}[htbp]
  \centering
...
have not yet been computed. What can be stated from the current analysis is descriptive: the tested quantum
configurations have higher observed mean final values and lower coefficients of
variation than the tested classical configurations. The current analysis does
not establish that the quantum parameterization causes these differences.
\textcolor{red}{REVIEW: before making an inferential classical-versus-quantum
claim, define the primary endpoint, perform the corresponding between-model
test with an effect-size and uncertainty estimate, and state how the unpaired
seed design affects interpretation.}
...
computed after the stated transaction costs; and (iv) whether the benchmark
figure uses exactly the same evaluation protocol.}
