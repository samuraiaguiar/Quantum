...
classical block is replaced by a PQC with the same input and output width; and (iv) how
the quantum configuration behaves across the tested register widths. These are
controlled comparisons within the present testbed; they are not designed to establish
...
The universe consists of ten equities traded on the Brazilian exchange (B3): ABEV3,
BBAS3, BBDC4, ITUB4, LREN3, PETR4, PRIO3, RENT3, VALE3 and WEGE3, with daily closing,
high and low prices retrieved from Yahoo Finance. Prices are normalised per asset with a
maximum-absolute scaler fitted exclusively on the training window and applied unchanged
to the test window; in addition, each observation window is normalised inside the
environment by its last closing price. A single observation is therefore a tensor of
shape
...
The chronological split uses training from 2017-01-01 to 2019-12-31 and an evaluation
window from 2020-01-01 to 2020-12-31. The evaluation window yields
...
Two properties of this testbed delimit the scope of the empirical results. The test
window is a single year, and an atypical one, containing both a sharp drawdown and a
strong recovery; the trajectories in \ref{sec:results} therefore describe behaviour
under one realised market path, not an expectation over market regimes. No separate
held-out validation partition is used: the evaluation environment also serves for
periodic validation, and online learning is enabled during the evaluation episode. Both
choices are inherited from the reference framework and are retained so that all models
--- baseline, classical and hybrid --- are evaluated under the same procedure.
Accordingly, the reported values should be interpreted as outcomes for this evaluation
protocol and this realised market path, rather than as an estimate of performance across
market regimes.
...
model-selection step. If so, the manuscript should avoid calling the 2020 window a
strictly held-out out-of-sample test set.}
...
Because RL training can vary across initialisations, no configuration is characterised
by a single run. Each configuration is trained \num{20} times independently, drawing
from a single pseudorandom stream seeded at the start of the batch and not reinitialised
between runs, so that each run begins from a distinct initialisation and the batch as a
whole is reproducible by re-executing the procedure. Because architectures with
different parameter counts consume the pseudorandom stream differently, initialisations are \emph{not} paired across models, and all comparisons between configurations are
...
The quantity reported throughout is the portfolio value along the 2020 evaluation
window, summarised at the last step as the \emph{final value}. Figures show the mean
trajectory with a $\pm 1$ standard-deviation envelope across runs. Dispersion is
summarised through the coefficient of variation (CV), alongside the mean and standard
deviation across runs. The CV is used as a scale-normalised description of dispersion
because configurations can differ in mean final value; it is not treated by itself as
...
corrected. If no inferential test was performed, keep all claims descriptive and avoid
terms such as statistically significant.}
...
batch size to every architecture, and is the protocol under which all proposed variants
are evaluated; the protocol is therefore designed to reduce procedural differences
between the proposed variants. This control supports an architectural comparison, but
does not by itself establish that every observed difference is caused solely by the
latent block. The \emph{original configuration} reproduces the hyperparameters published
for the reference model~\cite{RLportfolio}, and is used to characterise the reference
implementation under its published training conditions.
...
In the sweep over the width of the quantum register, the seed is set as a function of
the width,
%
\begin{equation}
  \mathrm{seed}(Q) = 42 + Q,
...
\end{equation}
%
so that each register width is evaluated under a distinct fixed seed. Therefore,
register width and pseudorandom stream are confounded in this sweep and their effects
cannot be separated. The width experiment can therefore describe the observed behavior
of the tested configurations, but it cannot isolate a causal effect of register width.
...
This section defines the evaluated architectures and the sequence of modifications used
to introduce an interchangeable latent block. The purpose of the sequence is to make the
...
The baseline is EI3 as reproduced from the reference implementation~\cite{RLportfolio}
and shown in \ref{fig:arch-ei3}. The observation tensor is processed by three parallel
heads; their outputs are concatenated with the previous portfolio weights and reduced by
a $1\times1$ convolution directly to the eleven allocation scores, which a softmax turns
into weights.
...
The property that makes EI3 a suitable point of departure is precisely the one that
makes it unsuitable as a host: the $1\times1$ convolution maps features to allocation
scores in a single step, so the network has no intermediate representation whose width
is a free parameter. There is no site at which a block of $Q$ neurons --- or $Q$ qubits
--- could be inserted without changing the meaning of the surrounding layers.

\subsection{Design constraints imposed by a quantum latent block}
\label{subsec:design-constraints}

Creating such a site requires three design changes in the implementation used here. They
are introduced to provide a compatible interface for the latent block, not as evidence
that this restructuring is uniquely required for quantum models.
...
distinct market states onto the same rotation angle, and the resulting many-to-one map
would be invisible to the loss while corrupting the representation. Restricting the
encoded angles to this interval limits the range presented to the periodic encoding.
...
the encoding injective without a precise statement of the encoding map and its domain;
the current implementation primarily bounds the angles.}

\paragraph{A decision head that consumes the block output.}
Because the block emits $Q$ values in $[-1,1]$ rather than eleven allocation scores, a
small MLP head is required to map the latent code back to the simplex. This head is
...
representation around the block, combined with its output through a learnable gate. It
was removed deliberately in the experimental design. With a bypass in place, the
decision head can receive the pre-block representation directly, allowing the gated path
to reduce its dependence on the block under study. Removing the branch ensures that the
decision head receives the latent-block output without this explicit bypass. This makes
the latent block an information bottleneck in the implemented architecture.
\textcolor{red}{REVIEW: the statement that removing the residual branch makes
optimisation harder is plausible but is not established by the experiments reported
here. Retain such a claim only if supported by a residual-vs-no-residual ablation.}
...
experimental design choice. Do not claim that training becomes infeasible without
presenting the supporting experiment.}

\subsection{The scaffold: QEI3-eq NoCL}
\label{subsec:arch-nolayer}

Applying the changes above without inserting any latent block yields the scaffold,
denoted QEI3-eq NoCL and shown in \ref{fig:arch-nolayer}: the convolutional block feeds
the latent projection, the scaled latent code feeds the MLP head directly, and the
softmax produces the allocation. This variant exists to answer a question that must be
settled before any quantum claim can be made --- whether the restructuring itself,
independently of the block, accounts for the behaviour observed later.
...
as a $Q\times Q$ linear transformation followed by a $\tanh$ activation. The activation
is chosen to match the range of the quantum readout: since $\langle Z_i \rangle \in
[-1,1]$ by construction, a classical surrogate with unbounded output would differ from
the quantum block in codomain as well as in parameterisation, and the ablation would no
longer isolate a single factor. At the reference width of eight, the block contributes
...
diagrams against the implementation. If the diagrams were generated directly from
n\_qubits, regenerate them using a neutral latent-width label for the classical model
and Q only for the quantum register.}
...
circuit, preserving input dimension, output dimension and position in the network. The
$Q$ scaled latent values are written into a $Q$-qubit register by angle embedding, a
RealAmplitudes ansatz with one repetition and full entanglement is applied, and the
block is read out through $\langle Z_i \rangle$ on each qubit.
\ref{fig:circuit-q2,fig:circuit-q8} show the circuit at the extremes of the sweep.

At the reference width of eight the circuit carries \num{16} trainable parameters,
against \num{72} for the classical block it replaces. This parameter-count asymmetry is
part of the implemented comparison and should be retained when interpreting any
...
PyTorch network through the \texttt{TorchConnector} interface, so that the hybrid model
is trained end-to-end by a single optimiser. Expectation values are computed exactly
with the
...
from a dedicated pseudorandom generator seeded independently of the training seed.
Circuit and network parameters are updated jointly with AdamW.

\paragraph{Simulation regime.}
All quantum experiments reported in this article use exact statevector expectation
values. This defines the simulation setting evaluated here; the study does not attempt
to characterise execution on quantum hardware. The corresponding limitation is stated in
\ref{sec:limitations}.
...
The results are presented in the order of the experimental comparisons: the baseline,
the scaffold, the classical block, the quantum width sweep, and the matched-width
comparison. The sequence makes the changes between configurations explicit; because not
all campaigns are seed-paired, it should not be read as a causal decomposition of
performance.
...
strongest EI3 configuration measured in this experiment; this does not establish that it
is the globally optimal EI3 training configuration.

Second, the run-to-run dispersion is substantial in the evaluated EI3 configurations:
standard deviations of \num{31608}, \num{28132} and \num{55409}, the last corresponding
to a coefficient of variation of roughly \num{0.25}. The variation across the 20 runs
motivates reporting repeated-run distributions rather than relying on a single trained
agent.
...
The scaffold has a mean final value of \num{280563} monetary units, above the highest
mean observed for the three EI3 configurations reported above. This comparison is
descriptive rather than causal, and the scaffold also exhibits substantial run-to-run
dispersion: the standard deviation across runs is \num{82410}, a coefficient of
...
The run-by-run trajectories visually share several market-driven features while
differing in level. However, trajectory similarity does not establish that the
underlying policies are qualitatively equivalent.
\textcolor{red}{REVIEW: if the manuscript intends to claim that the runs learn
policies of a common shape or that initialization primarily rescales a common policy,
this requires a quantitative policy/trajectory-similarity analysis. Without such an
analysis, retain only the descriptive statement about the observed dispersion.}
...
the block at the reference width of eight neurons, chosen to match the register width of
...
between-configuration inferential test for this comparison, so no statistical difference
is claimed.
...
test and effect-size estimate for QEI3-eq NoCL versus QEI3-eq if an inferential claim
about the classical latent block is intended.}
...
for the scaffold and \num{0.22} for its classical instantiation. These values are lower
than those observed for the scaffold and the tested classical latent block under the
reported protocol. The present experiment does not identify the mechanism responsible
for this difference.

The two-qubit circuit uses fewer circuit parameters and entangling gates than the wider
circuits (four parameters and one CNOT, compared with sixteen parameters and
twenty-eight CNOTs at eight qubits). Since the final-value distributions are not
distinguished in this sweep, the results do not provide evidence that the wider
registers improve the reported portfolio metric.
\textcolor{red}{REVIEW: avoid calling the two-qubit model a cost--benefit optimum
unless computational cost is measured with a defined metric and included in the
comparison.}
...
The two instantiations of the latent block can now be compared at the same width, under
the same protocol, inside the same network. At eight units, the classical block reaches
a mean final value of \num{301294} with a coefficient of variation of \num{0.22}; the
quantum block reaches \num{369224} with a coefficient of variation of approximately
\num{0.030}, using \num{16} trainable parameters against \num{72}.

\ref{tab:summary} collects every campaign reported above on a common scale.
Across the configurations listed in the table, the observed means and dispersions follow
an ordered pattern. Because the campaigns are not fully seed-paired and the
architectures differ in parameterization, this ordering is reported descriptively and is
not interpreted as a causal progression.
\begin{table}[htbp]
  \centering
...
have not yet been computed. What can be stated from the current analysis is descriptive:
the tested quantum configurations have higher observed mean final values and lower
coefficients of variation than the tested classical configurations. The current analysis
does not establish that the quantum parameterization causes these differences.
...
% GPT-REVIEW: The following issues remain intentionally visible because the
% current manuscript/data do not support stronger statements.
\textcolor{red}{REVIEW: before submission, verify (i) whether the 2020
environment participates in model selection or online updating; (ii) the exact
statistical tests and effect sizes for the classical-vs-scaffold and
classical-vs-quantum comparisons; (iii) whether all reported monetary values are
computed after the stated transaction costs; and (iv) whether the benchmark figure uses
